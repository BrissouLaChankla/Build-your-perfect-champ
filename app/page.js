...
export default function Home() {
  ...
  const selectChamp = async () => {
    setShuffling(true);
    setTimeout(() => setShuffling(false), 1600);
    const response = await fetch(
      `https://ddragon.leagueoflegends.com/cdn/14.7.1/data/${all.getLanguage()}/champion.json`
    );
    const data = await response.json();
    setChampSelected(all.getRandomChamps(data.data));
  };

  const selectAbility = (key, name, spellName, spellDesc) => {
    setAbilitiesSelected([
      ...abilitiesSelected,
  ...
  const pictureOrLetter = (letter) => {
    ...
    if (ability) {
      const type = letter === "P" ? "passive" : letter === "D" ? "champion" : "spell";
      return (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <img
            src={`https://ddragon.leagueoflegends.com/cdn/14.7.1/img/${type}/${ability.name}.png`}
            className="object-cover"
            width={100}
            height={100}
            alt={`${ability.spellName} icon`}
          />
        </motion.div>
      );
    }
  ...
  return (
    <>
      ...
      <main style={bgStyle}>
        <div
          className="p-4 flex flex-col items-center max-w-7xl m-auto"
        ...
        >
          ...
          <div className="grid grid-cols-12 my-8 grow w-full">
            {champSelected ? (
              <>
                ...
                <div className="col-span-12 md:col-span-5">
                  <Ability
                    ...
                    abilitiesSelected={abilitiesSelected}
                    name={champSelected.name}
                    shuffling={shuffling}
                  />
                </div>
              ...
              <div className="col-span-12 flex flex-col items-center justify-center gap-4 text-center">
                <p className="text-white/80 max-w-xl text-sm md:text-base">
                  Pick one ability at a time from random champions and fuse them into your own legend.
                </p>
                <button className="btn btn-primary btn-lg" onClick={selectChamp}>
                  Start Building
                </button>
              </div>
            )}
          ...
          <dialog id="my_modal_4" className="modal">
            <div className="modal-box w-11/12 max-w-5xl">
              ...
              <div className="flex flex-col">
                {abilitiesSelected
                  .sort((a, b) => all.order.indexOf(a.key) - all.order.indexOf(b.key))
                  .map((el, i) => {
                    const type =
                      el.key === "P" ? "passive" : el.key === "D" ? "champion" : "spell";
                    return (
                      <motion.div
                        key={el.key}
                      ...
                      >
                        <div className="flex items-center gap-3 border-b-2 p-2">
                          <img
                            src={`https://ddragon.leagueoflegends.com/cdn/14.7.1/img/${type}/${el.name}.png`}
                            className={`skeleton ${i === 0 ? "rounded-full" : i === all.order.length - 1 ? "mask mask-hexagon" : "rounded-lg"} w-[60px] h-[60px] object-cover`}
                            width={50}
                            height={50}
                            alt={el.name}
                          />
                          <div className="grow">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="h-5 w-5 flex items-center justify-center rounded bg-gray-700 text-xs">
                                {el.key}
                              </span>
                            </div>
                            <p
                              className="text-xs lg:text-sm line-clamp-2 mt-0.5"
                              dangerouslySetInnerHTML={{ __html: el.spellDesc }}
                            ></p>
                          </div>
                        </div>
                      </motion.div>
          ...
          {!champSelected && (
            <div className="fixed bottom-0 text-xs md:text-sm flex items-center gap-2 rounded-t-lg backdrop-blur-lg bg-black/30 px-4 p-2">
