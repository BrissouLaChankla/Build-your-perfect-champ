    ...
    return (
        <div className="border-2 h-full p-4 flex flex-col items-center justify-center">
            ...
            <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="mb-8 h-14 -mt-8 md:mb-0 bg-gray-900 px-4 py-2 rounded-lg md:mt-4 text-2xl lg:text-4xl font-bold"
            >
                {shuffling ? <ScrambleText text={`Ef${name}Zf`} scrambleInterval={100} /> : name}
            </motion.div>
        </div>
    );
}
